...
import type { DailyReportFilterBounds } from "@/lib/daily-report-filters"
import {
...
export async function getDailyReportData(
  ...
  for (const row of brandApprovalRows.rows) {
    ...
    if (isContentReportFullyApproved(row.supervisor_status, row.director_status)) {
    ...
    .map((summary) => ({
      ...
      completionRate:
        summary.gradedTotal > 0
          ? Math.round((summary.gradedDone / summary.gradedTotal) * 10000) /
            100
          : 0,
    }))
    .filter((summary) =>
  ...
  const approvalLog: DailyApprovalLogEntry[] = approvalRows.rows.map(
    (row) => ({
      id: row.id,
      ...
      scheduledPublishedDate:
        row.scheduled_published_date?.toISOString() ?? null,
    })
  )
