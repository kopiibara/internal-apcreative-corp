...
import { countGradedAssignmentMetrics } from "@/lib/daily-report-metrics"
import type {
  TaskPriority,
  TaskProofType,
  TaskType,
...
export async function getStaffAccountabilityData({
  ...
  for (const row of brandApprovalRows.rows) {
    ...
    if (row.supervisor_status === "Rejected" || row.director_status === "Rejected") {
    ...
    if (row.publish_status === "Scheduled" || row.publish_status === "Published") {
