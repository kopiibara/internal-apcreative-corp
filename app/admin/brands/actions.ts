...
export async function createBrand(
  ...
  try {
    ...
    if (isUniqueViolation(error)) {
      return { success: false, message: "A brand with this slug already exists." }
    }
...
export async function deleteBrand(input: {
  ...
  try {
    const deleted = await transaction(async (client) => {
      const dependencyCounts = await getBrandDependencyCounts(parsed.data.brandId)
    ...
    if (!deleted) {
      return {
        success: false,
        message: "This brand has related records. Please deactivate it instead.",
      }
    }
